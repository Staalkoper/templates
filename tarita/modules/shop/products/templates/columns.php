            ...
            <?php } ?>
                <div class="mw-ui-col">
                    <div class="mw-ui-col-container">
                        <div class="mw-module-products-default-item product-item-single" itemscope
                             itemtype="<?php print $schema_org_item_type_tag ?>">
                            <?php if ($show_fields == false or in_array('thumbnail', $show_fields)): ?>



                                <a class="img-polaroid img-rounded" href="<?php print $item['link'] ?>"> <span
                                        class="valign"> <span class="valign-cell"> <img
                                                <?php if ($item['image'] == false){ ?>class="pixum"<?php }else{ print "class='img-responsive'";} ?>  i
                                                src="<?php print thumbnail($item['image']); ?>"
                                                alt="<?php print $item['title'] ?>"
                                                title="<?php print $item['title'] ?>" temprop="image"/> </span> </span>
                                </a>
                            <?php endif; ?>
                            <?php if ($show_fields == false or in_array('title', $show_fields)): ?>
                                <h3 itemprop="name"><a itemprop="url" class="lead"
                                                       href="<?php print $item['link'] ?>"><?php print $item['title'] ?></a>
                                </h3>
                            <?php endif; ?>
                            ...
                            <div class="product-price-holder clearfix">
                                ...
                                <?php if ($show_fields == false or in_array('add_to_cart', $show_fields)): ?>
                                    ...
                                    <?php if (is_array($item['prices'])): ?>
                                        <button class="mw-ui-btn" type="button"
                                                onclick="mw.cart.add('.mw-add-to-cart-<?php print $item['id'] . $count ?>');">
                                            <i class="icon-shopping-cart glyphicon glyphicon-shopping-cart"></i>&nbsp;<?php print $add_cart_text ?>
                                        </button>
                                    <?php endif; ?>
                                <?php endif; ?>
                            </div>
                            <?php if (is_array($item['prices'])): ?>
                                <?php foreach ($item['prices'] as $k => $v): ?>
                                    <div
                                        class="clear products-list-proceholder mw-add-to-cart-<?php print $item['id'] . $count ?>">
